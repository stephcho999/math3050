...
def getstoch(df, date):
    if date in df.index:
        return df.loc[date, 'stoch']
    else:
        return False
...
def getk(df, date):
    if date in df.index:
        return df.loc[date, 'K']
    else:
        return False


def stoch_target(df_list, date):
    target_list = []
    for df in df_list:
        val = getstoch(df, date)
        k = getk(df,date)
        if val == True:
            if k <= 20:
...
def get_next_date(curr_date, days, df=xlv_df):
...
def one_cycle_buy_sell(funds, fund_df, df_list, date, days):
    ...
    #sell
    date = get_next_date(date, days, df)
...
def simulate(funds, df_list, start, days, cycle):
    ...
    for i in range(cycle):
        target_list = stoch_target(df_list, date)
